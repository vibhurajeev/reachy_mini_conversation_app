...
| D13 | Deployment topology (user decision, 2026-07-17; **revised to v2 same day, built**) | **v2 — split: thin app ON the robot + bridge service on the VM.** The fork installs on the Pi as a normal daemon-managed app (`CONVERSATION_BACKEND=bridge`): mic capture, Silero VAD (ONNX, no torch), the 60 Hz motion loop, and directive execution all run locally; each endpointed utterance is POSTed to the `avail-intern-bridge` FastAPI service on the VM (port 8643, `BRIDGE_API_KEY` bearer), which does STT → Hermes (localhost) → directive parsing → TTS and streams back ndjson events with ready-to-play audio. Only speech crosses the network, as discrete requests; motion jitter risk is gone; Hermes's api_server stays localhost-only; the robot keeps the official app UX (dashboard, `--startup-app`, antenna wake). Offline fallback: a packaged pre-synthesized wav so bridge-unreachable is never silent. The v1 remote-session mode (below) remains available unchanged via `CONVERSATION_BACKEND=hermes`. *(v1 text kept for history:)* The app runs server-side, co-located with Hermes — the Pi runs only the stock Reachy daemon. The SDK is location-agnostic: mic/speaker/camera stream over WebRTC (~100 ms) and motion commands go over the LAN, exactly like the Lite variant. Consequences: STT/TTS upgrade to server-class models (faster-whisper `small`/`large-v3-turbo`, Kokoro-82M) — the Pi-feasibility constraint is void; bridge↔Hermes is localhost; one deploy target; phase-3 vision gets frames where the compute is. Requirements: VM must reach the robot's LAN (Tailscale/on-prem); **day-1 spike: validate motion smoothness + audio jitter with the app remote over the real office Wi-Fi**. Fallback (same code, config-only): run the app on the Pi with the lightweight model set (Silero + whisper-base int8 + Piper) if Wi-Fi proves unreliable — keep STT/TTS pluggable so both profiles work. |
